...
| Gemini | AI answer | **MEASURED** | 24/24 | Signed-in free consumer UI, Temporary chat, `Flash` UI label; exact queries; dated full-answer hashes, AX hashes, screenshots, and all extracted source URLs retained. One SHAR mention and zero direct valid citations were observed. |
...
`AI-SEARCH-MEASUREMENT-WAVE-03-2026-09-06.json` records Gemini Flash at 24/24 `MEASURED` in signed-in Temporary chats on the free consumer UI. One answer mentioned SHAR Production, with Dprofile among its sources, but no answer supplied a direct SHAR-controlled URL; it is therefore `MENTION_ONLY`, with zero valid citations. The wave retained 59 unique extracted source URLs and private full-answer, AX and screenshot evidence. ChatGPT, Claude and Perplexity remain `NOT_MEASURED` until their named consumer surfaces are accessible and identifiable under this protocol.
